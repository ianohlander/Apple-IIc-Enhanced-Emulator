...
export class CSharpApple2Compiler {
  public static compile(sourceCode: string): ModernCodeCompilationResult {
    ...
    const asmOutput: string[] = ['; --- Generated 65C02 Machine Code from C# Source ---', '* = $2000', 'MAIN:'];
  ...
  private static compileLine(line: string, bin: number[], asm: string[], logs: string[]): void {
    ...
    if (/SetDoubleHiRes|DoubleHiRes/.test(line)) {
      CSharpApple2Compiler.emitDoubleHiRes(bin, asm, logs);
      ...
      CSharpApple2Compiler.emitBeep(bin, asm, logs);
    }
  }
  ...
  private static emitBeep(bin: number[], asm: string[], logs: string[]): void {
    ...
    asm.push('       LDX #$60', 'BEEP:  LDA $C030', '       LDY #$30', 'DLY:   DEY', '       BNE DLY', '       DEX', '       BNE BEEP');
  }
}
